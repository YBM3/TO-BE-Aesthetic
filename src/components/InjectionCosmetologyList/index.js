...
function InjectionCosmetologyList({isMenuOpen}) {
  ...
  const listRefs = InjectionCosmetologyData.map(() => createRef());

  const contentRefs = InjectionCosmetologyData.map((section) =>
    section.items.map(() => createRef())
  );
  useLayoutEffect(() => {
    const timer = setTimeout(() => {
      setHeights(
        InjectionCosmetologyData.map((section, sectionIndex) =>
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setListHeights(
        InjectionCosmetologyData.map((_, index) =>
  ...
  return (
    ...
    style={{
      maxHeight: expandedSection !== null ? `${listHeights.reduce((a, b) => a + b, 0)}px` : "590px",
    }}
  >
      {InjectionCosmetologyData.map((section, index) => (
...
export default InjectionCosmetologyList;
